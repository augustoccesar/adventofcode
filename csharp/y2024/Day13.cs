...
[RunnableDay(year: 2024, day: 13)]
class Day13 : Day
{
    public override string PartOne()
    {
        long tokens = 0;
        foreach (var machine in ReadMachines(ReadInputLines(), 0))
        {
    ...
    {
        long tokens = 0;
        foreach (var machine in ReadMachines(ReadInputLines(), 10_000_000_000_000))
        {
            if (Winnable(machine, out var price))
            {
                tokens += price;
    ...
    private static List<Machine> ReadMachines(string[] inputLines, long addedPrizePos)
    {
        List<Machine> machines = [];
        for (int i = 0; i < inputLines.Length; i++)
        {
            var buttonALine = inputLines[i];
            var buttonALineTokens = buttonALine.Split(separators).ToList();

            var buttonBLine = inputLines[i + 1];
            var buttonBLineTokens = buttonBLine.Split(separators).ToList();

            var prizeLine = inputLines[i + 2];
