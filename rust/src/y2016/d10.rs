...
impl Day for Day10 {
    ...
    fn part_one(&self) -> String {
        execute(self.read_default_input(), [61, 17]).0
    }

    fn part_two(&self) -> String {
        let (_, output_map) = execute(self.read_default_input(), [0, 0]);
...
fn execute(input: String, watch_pair: [i8; 2]) -> (String, HashMap<String, Vec<i8>>) {
    ...
    while operations.iter().any(|(_, v)| !*v) {
        ...
        for (operation, executed) in operations.iter_mut().filter(|(_, v)| !**v) {
            if let Some(c) = REG_BOT_VALUE.captures(operation) {
                let bot_key: &str = &c[2];
            ...
            if let Some(c) = REG_BOT_TRANSFER.captures(operation) {
                let bot_key: &str = &c[1];
    ...
    (watched_bot.to_string(), output_map)
}
