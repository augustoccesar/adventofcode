...
import com.augustoccesar.aocjava.y2019.shared.intcomputer.exceptions.UnsupportedOpCode;
import com.augustoccesar.aocjava.y2019.shared.intcomputer.exceptions.UnsupportedParameterMode;
...
public class IntComputer {
  ...
  @Getter private Memory memory;

  @Getter private long instructionPointer = 0;

  @Getter private long relativeBase = 0;

  @Getter private boolean halted = false;

  @Getter private boolean paused = false;

  @Getter private InputSource inputSource;

  @Getter private final LinkedList<Long> output = new LinkedList<>();

  @Getter private final List<Instruction> ranInstructionsLog = new ArrayList<>();
    ...
    for (int i = 0; i < instruction.getOpCode().getParamSize(); i++) {
      ...
      if (instruction.isParamWriteToMemory(i)) {
        switch (parameter.getMode()) {
          ...
          case IMMEDIATE ->
              throw new RuntimeException("Write to memory parameter shouldn't be IMMEDIATE");
        ...
        switch (parameter.getMode()) {
          ...
          case RELATIVE ->
              parameters.add(
                  this.memory.readOrDefault(parameter.getValue() + this.relativeBase, 0));
          default -> throw new UnsupportedParameterMode(parameter.getMode());
        }
      }
    }
