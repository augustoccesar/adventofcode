...
enum PanelColor {
  ...
  @Getter private final int intRepr;

  @Getter private final char charRepr;
...
class Robot {
  private static final Integer[][] moveModifier =
      new Integer[][] {
        new Integer[] {0, 1}, // up
  ...
  @Getter private final Map<String, PanelColor> positionTracker = new HashMap<>();
  ...
  public void printPositionHistory() {
    List<Integer[]> positions =
        this.positionTracker.keySet().stream()
            .map(
                it -> {
                  String[] tokens = it.replace("(", "").replace(")", "").split(",");
