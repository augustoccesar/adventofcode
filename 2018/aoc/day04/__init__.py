...
class Day04(Task):
    ...
    def part_two(self) -> str:
        ...
        guards_each_max = {k: max(v.items(), key=operator.itemgetter(1)) for k, v in times.items()}
        max_of_all = max(guards_each_max.items(), key=lambda item: item[1][1])  # (guard_id, (minute, amount))
...
def guard_highest_minute_occurrence(registry_ranges: List[Tuple[int, int]]) -> int:
    ...
    """
    return max(guard_times_occurrences(registry_ranges).items(), key=operator.itemgetter(1))[0]
